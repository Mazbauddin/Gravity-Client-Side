      ...
      >
        ...
        <div className="fixed inset-0 overflow-y-auto">
          <div className="flex min-h-full items-center justify-center p-4 text-center">
            ...
            >
              <DialogPanel className="w-full h-56 max-w-md transform overflow-hidden rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-500 p-6 text-left  align-middle shadow-xl transition-all">
                <DialogTitle
                  as="h3"
                  className="text-lg font-medium text-center leading-6 text-gray-900"
                >
                  Update Employee Or HR Role
                </DialogTitle>
                      ...
                      >
                        <ListboxOptions className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                          {roles.map((role, roleIdx) => (
                            <ListboxOption
                              key={roleIdx}
                              className="relative cursor-default select-none py-2 pl-10 pr-4 text-gray-900 data-[focus]:bg-blue-700  data-[focus]:text-white"
                              value={role}
                            >
                              {({ selected }) => (
                                <>
